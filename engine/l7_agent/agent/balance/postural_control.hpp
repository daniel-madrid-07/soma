...
// controlador integra la señal IMU (inclinación + velocidad) y reparte
// ACTIVACIÓN entre los actuadores del tobillo (posterior/anterior) para corregir.
// No emite par: la fuerza sale del modelo no lineal.
//
...
struct BalanceController {
    ...
    Output command(Real tilt, Real rate) const {
        ...
        Real post = u > 0 ? u : 0.0;      // actuador posterior (recupera de caída adelante)
        Real ant = u < 0 ? -u : 0.0;      // actuador anterior (recupera de caída atrás)

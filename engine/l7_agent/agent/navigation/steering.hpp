...
//
// El cuerpo gira hacia DONDE APARECE el objetivo en la sensor. No lee la posición
// del objetivo en el mundo: solo su coordenada retiniana (u). Si el objetivo está a
// la derecha en la sensor, gira a la derecha, hasta centrarlo en la fóvea. Combinado
// con el avance del caminante (Fase 6), el cuerpo camina hacia lo que ve.
//
//   cámara (imagen) → u en la sensor → error de rumbo = atan2(u, focal) → giro
//
...
struct Steering {
    ...
    static Real bearing_from_sensor(Real u, Real focal) {

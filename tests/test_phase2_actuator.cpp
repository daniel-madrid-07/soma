// SOMA — Test FASE 2: el ACTUADOR mueve el HUESO.
//
// Primer momento mecánico. Un hueso cuelga de un pivote. Un actuador no lineal va de
// un punto fijo por encima del pivote a una inserción en el hueso. Al ACTIVARLO,
// el actuador se contrae, tira de la inserción y FLEXIONA el hueso hacia arriba.
// Cero animación: activación -> fuerza no lineal -> par -> el hueso rota por física.
//
// Además valida las propiedades del modelo no lineal (curvas F-L, F-V, pasiva).
...
static void test_actuator_properties() {
    SpringActuator m;
...
static Real run_with_activation(Real activation) {
    ...
    SpringActuator act;
    act.f_max = 800; act.l_opt = 0.6; act.v_max = 10;
    ...
    for (int step = 0; step < 4000; ++step) {      // 4 s
        ...
        actuators::apply_actuator(act, at, anchor, bone);
...
int main() {
    test_actuator_properties();

    // Respuesta graduada: a más activación, más flexión (menor ángulo).
    // Esto prueba que es el ACTUADOR quien mueve el hueso, no un truco.
